"""Collector that runs arp-scan and captures structured artifacts."""
...
def _validate_scope(config: dict[str, Any]) -> tuple[str, list[str], list[str]]:
    ...
    if not interface:
        raise ValueError("config.interface is required for arp-scan")
...
def collect_arp_scan(config: dict[str, Any], *, run_id: str, run_root: Path) -> CollectorRecord:
    """Run arp-scan safely with explicit arguments from configuration."""

    interface, allowed_cidrs, scanned_cidrs = _validate_scope(config)
    binary = str(config.get("binary", "arp-scan"))
